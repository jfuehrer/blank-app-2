...
def create_results_dataframe(data):
    # select relevant columns to create a summary dataframe
    results_df = data[['vendor_name', 'foreign_labor_risk_score', 'risk_category', 'interpretation']]
    print("\nSummary of Results:\n")
    print(results_df.head()) # display the top few rows as a preview
    return results_df
...
def main():
    ...
    data['permanent_visa_score'] = data.apply(
        lambda row: calculate_permanent_visa_score(
            ...
            {
                'certified_trend': row['certified_trend'],
                'denied_withdrawn_trend': row['denied_withdrawn_trend']
            }
        ), axis=1  
    )
    ...
    results_df = create_results_dataframe(data)
